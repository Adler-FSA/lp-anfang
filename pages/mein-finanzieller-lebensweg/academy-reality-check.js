...
const parseNum=s=>{const m=String(s||'').match(numberPattern);return m?Number(m[0].replace(/\./g,'').replace(',','.')):0};
const money=n=>new Intl.NumberFormat('de-DE',{style:'currency',currency:'EUR',maximumFractionDigits:0}).format(Number(n)||0);

function originalScope(){return $('#resultArea')||$('main')||document.body;}
function isGenerated(node){return !!node.closest('#academyFinal,#academyRealityCheck,#academySetupBridge,#printBook,.backup-panel');}
function readValue(label){
  const scope=originalScope();
  const nodes=$$('div,article,section,li,tr,td,span,strong',scope)
    .filter(n=>!isGenerated(n)&&norm(n.textContent).includes(label))
    .sort((a,b)=>norm(a.textContent).length-norm(b.textContent).length);
  for(const n of nodes){
    const t=norm(n.textContent);
    const pos=t.indexOf(label);
    if(pos<0)continue;
    const rest=t.slice(pos+label.length);
    const m=rest.match(new RegExp(`${numberPattern.source}\\s*(?:€(?:\\s*\\/\\s*Monat)?|%)`));
    if(m)return norm(m[0]).replace(/\s*\/\s*/g,'/');
  }
  return '—';
}
function hideLegacyExplanation(host){
  $$(':scope > details',host).forEach(d=>{d.hidden=true;d.setAttribute('aria-hidden','true');});
}
function build(){
  const host=$('#academyFinal');
  if(!host)return;
  hideLegacyExplanation(host);
  let sec=$('#academyRealityCheck');
  if(!sec){
    sec=document.createElement('section');
    sec.id='academyRealityCheck';
    sec.className='academy-reality-check';
    const bridge=$('#academySetupBridge');
    bridge?host.insertBefore(sec,bridge):host.append(sec);
  }
  ...
  const free=parseNum(freeRaw);
  const required=parseNum(requiredRaw);
  const capital=parseNum(capitalRaw);
  const coverage=parseNum(coverageRaw);
  ...
  const wishExplanation=wishesRaw==='—'
    ?'Die ausgewählten Wünsche werden im Simulator nach ihrer jeweiligen Kostenart, ihrem Zielalter und ihrer Priorität berücksichtigt.'
    :`Die Anzeige ${wishesRaw} ist eine modellierte monatliche Belastungsgröße. Sie darf nicht so verstanden werden, als wären alle Wünsche heute gleichzeitig fällig. Einmalige, jährliche und monatliche Wünsche werden im Lebensweg unterschiedlich eingeordnet.`;
  sec.innerHTML=`
    ...
    <div class="arc-grid">
      ...
      <article><span>Modellierter Kapitalbedarf</span><strong>${esc(capitalRaw)}</strong><p>Die Zahl ist kein Kontostand, den du morgen besitzen musst. Sie zeigt die modellierte Größenordnung hinter Lebensstandard, Ruhestand und ausgewählten Zielen.</p></article>
    ...
    <div class="arc-truth"><h3>Der entscheidende Realitätscheck</h3><p><strong>Mit einem monatlichen Defizit lässt sich kein Vermögen aufbauen.</strong> Ein modellierter Kapitalbedarf von <strong>${esc(money(capital))}</strong> kann nicht durch Hoffen, gelegentliches Sparen oder einzelne Anlagen geschlossen werden. Nach den aktuellen Modellwerten müsste sich dein monatlicher finanzieller Spielraum um ungefähr <strong>${esc(money(monthlyChange))}</strong> verbessern: zuerst um das heutige Minus von <strong>${esc(money(Math.max(0,-free)))}</strong> zu schließen und anschließend die erforderliche Rücklage von <strong>${esc(money(required))}</strong> tragen zu können.</p><p>${esc(wishExplanation)}</p><p>Diese Größenordnung ist keine persönliche Empfehlung und keine Garantie. Sie zeigt aber ehrlich, wie weit Wunsch und heutige Realität auseinanderliegen.</p></div>
...
let timer=0;
const refresh=()=>{clearTimeout(timer);timer=setTimeout(build,220)};
...
document.addEventListener('click',e=>{if(/berechnen|musterwerte/i.test(e.target?.textContent||''))setTimeout(build,500)},true);
setTimeout(()=>{const result=$('#resultArea');if(result)new MutationObserver(refresh).observe(result,{subtree:true,childList:true,characterData:true});},1000);
})();
