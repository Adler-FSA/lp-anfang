...
function findRange(journey){
  ...
  return ranges.find(r=>/alter|lebensweg|zeitreise/i.test([r.id,r.name,r.getAttribute('aria-label'),r.closest('label')?.textContent].join(' ')))
    ||ranges[0]
    ||$$('input[type="range"]','#resultArea').find(r=>/alter/i.test([r.id,r.name,r.getAttribute('aria-label')].join(' ')));
}
function install(){
  if($('#fsaAgeSliderDock'))return;
  const journey=findJourney(),range=findRange(journey);
  ...
  style.textContent=`
    #fsaAgeSliderDock{margin:22px 0 4px;padding:18px 20px;border:1px solid #d8e5e8;border-radius:18px;background:linear-gradient(135deg,#f2fbfb,#fff 55%,#fff5fa);box-shadow:0 10px 24px rgba(19,34,56,.06)}
...
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>setTimeout(install,1300),{once:true});
