...
const KEY='fsa_finanzlebensweg_systemkern_v1';
let applying=false,timer=0,patching=false;
const norm=s=>String(s||'').replace(/\s+/g,' ').trim();
const slug=s=>norm(s).normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'')||'feld';
const parseNum=v=>{const m=String(v||'').match(/-?(?:\d{1,3}(?:\.\d{3})+|\d+)(?:,\d+)?/);return m?Number(m[0].replace(/\./g,'').replace(',','.')):0};
