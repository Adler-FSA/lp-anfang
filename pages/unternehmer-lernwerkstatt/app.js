...
const ULW_KEY='fsa_unternehmer_lernwerkstatt_v1';
function load(){try{return JSON.parse(localStorage.getItem(ULW_KEY))||{modules:{},lastSaved:''}}catch(e){return{modules:{},lastSaved:''}}}
function save(d){d.lastSaved=new Date().toISOString();localStorage.setItem(ULW_KEY,JSON.stringify(d));document.querySelectorAll('[data-save-state]').forEach(e=>e.textContent='Lokal gespeichert: '+new Date().toLocaleString('de-DE'))}
