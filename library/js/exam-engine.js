...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const state = { idx: 0, correct: 0, order: shuffle([...Array(pool.length).keys()]), answers: [] };
  ...
  function renderResult() {
    ...
    const color =
      points < 60 ? "#ef4444" : points < 75 ? "#cd7f32" : points < 90 ? "#93c5fd" : "#d4af37";
    const status =
      points < 60
        ? lang === "de"
          ? "Wiederholen ❌"
    ...
    document.getElementById("toPanelBtn")?.addEventListener(
      "click",
